...
import { getOrderById, getOrdersGroupedByCustomer, updateOrder, deleteOrder } from "@/lib/orders-db";
...
// One section per customer. Each section bundles their orders together
// with a merged "prendas pedidas" list (every item across their orders,
// quantities summed) — replaces the old separate orders/items views.
export async function GET() {
  if (!(await isAuthenticated())) {
    return jsonResponse({ error: "No autorizado." }, 401);
  }

  const customers = await getOrdersGroupedByCustomer();
